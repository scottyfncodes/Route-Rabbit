        ...
        return (
          <div key={stop.id}>
            {idx > 0 && stop.driveMinutesFromPrev > 0 && (
              <div className="flex items-center gap-2 pl-6 py-1.5 text-[13px] text-[#5c6966]">
                <span className="w-px h-4 bg-black/10 ml-[7px]" />
            ...
            <div
              className={`rounded-2xl border-2 px-4 py-3.5 flex items-center gap-3 ${
                isActive ? 'border-primary-500 bg-mint-50' : stop.kind === 'visit' ? 'border-black/5 bg-white' : 'border-dashed border-black/10 bg-transparent'
              }`}
            >
              <div className="text-[22px] leading-none shrink-0">{KIND_ICON[stop.kind]}</div>
              <div className="min-w-0 flex-1">
                <div className="flex items-baseline gap-2 flex-wrap">
                  <span className="font-bold text-[15.5px] text-[#132825] whitespace-nowrap shrink-0">
                    {formatTimeShort(stop.arrive)}
                    {stop.arrive !== stop.depart ? `–${formatTimeShort(stop.depart)}` : ''}
                  </span>
                  <span className="font-semibold text-[15px] text-[#132825] truncate">
                    {isVisit ? patient?.initials ?? stop.label : stop.label}
                  </span>
                </div>
                {isVisit && patient?.notes && <div className="text-[12.5px] text-[#7a8582] italic truncate">{patient.notes}</div>}
                {isVisit && patient?.address && <div className="text-[12.5px] text-[#5c6966] truncate">{patient.address}</div>}
              </div>
              {isVisit && patient && (
                <div className="flex flex-col gap-1.5 shrink-0">
                  ...
                  <button
                    ...
                    className="text-[11.5px] font-bold px-2.5 py-1.5 rounded-full bg-coral-50 text-coral-700"
                  >
                    Cancel
                  </button>
                </div>
