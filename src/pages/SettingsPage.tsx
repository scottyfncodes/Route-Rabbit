  ...
  return (
    <div className="flex-1 flex flex-col">
      <header className="px-4 pt-6 pb-3">
        <h1 className="text-[24px] font-extrabold text-[#132825]">Settings</h1>
      </header>

      <div className="flex-1 overflow-y-auto px-4 pb-8 space-y-6">
        <section>
          <h2 className="text-[13px] font-bold text-[#4a5a57] uppercase tracking-wide mb-2">Home base address</h2>
          <input
          ...
          />
          <p className="text-[12.5px] text-[#5c6966] mt-1.5">Used as the default start &amp; end location for new days.</p>
        </section>

        <section>
          <h2 className="text-[13px] font-bold text-[#4a5a57] uppercase tracking-wide mb-2">Typical driving speed</h2>
            ...
            {SPEED_PRESETS.map((s) => (
              <button
                ...
                className={`flex-1 py-3 rounded-xl text-[13px] font-semibold border-2 ${
                  settings.avgSpeedMph === s.value ? 'bg-primary-600 border-primary-600 text-white' : 'bg-white border-black/10 text-[#132825]'
                }`}
              >
                {s.label}
              </button>
          ...
          <p className="text-[12.5px] text-[#5c6966] mt-1.5">Used to estimate drive times between visits when building your route.</p>
        </section>

        <section className="bg-mint-50 border border-mint-200 rounded-2xl px-4 py-4">
          <h2 className="text-[15px] font-bold text-primary-800 mb-1.5">🔒 Privacy, by design</h2>
          <p className="text-[13.5px] text-[#3a4c49] leading-relaxed">
        ...
        <section>
          <h2 className="text-[13px] font-bold text-[#4a5a57] uppercase tracking-wide mb-2">Demo data</h2>
          <div className="bg-white rounded-2xl border border-black/5 px-4 py-4">
            <p className="text-[13.5px] text-[#5c6966] mb-3">
        ...
        <section>
          <h2 className="text-[13px] font-bold text-[#4a5a57] uppercase tracking-wide mb-2">Data</h2>
          <div className="bg-white rounded-2xl border border-black/5 px-4 py-4">
            {!confirmingReset ? (
              ...
              <div className="space-y-2">
                <p className="text-[13.5px] text-coral-700 font-medium">
        ...
        <p className="text-center text-[12px] text-[#9aa3a0] pt-2">Route Rabbit &middot; a free, tiny route-planning utility</p>
      </div>
    </div>
  )
}
